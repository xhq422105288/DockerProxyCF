/**
 * Cloudflare Worker — Docker Hub / GCR / registry.k8s.io / GHCR / Quay pull-only reverse proxy
 *
 ...
 * 通过 registry.k8s.io 前缀拉取 Kubernetes 公共镜像：
 *   docker pull <domain>/registry.k8s.io/pause:3.9
 * 通过 ghcr.io / quay.io 前缀拉取公共镜像：
 *   docker pull <domain>/ghcr.io/actions/actions-runner:latest
 *   docker pull <domain>/quay.io/prometheus/prometheus:latest
...
function resolveRegistryTarget(pathname) {
  ...
  if (pathname === '/v2/registry.k8s.io') {
    return { registry: REGISTRIES.k8s, upstreamPath: '/v2/', proxyPath: pathname };
  }
  if (pathname === '/v2/registry.k8s.io/' || pathname.startsWith('/v2/registry.k8s.io/')) {
    const upstreamPath = REGISTRIES.k8s.rewritePath(pathname);
    return { registry: REGISTRIES.k8s, upstreamPath, proxyPath: pathname };
  }
  if (pathname === '/v2/ghcr.io') {
    return { registry: REGISTRIES.ghcr, upstreamPath: '/v2/', proxyPath: pathname };
  }
  if (pathname === '/v2/ghcr.io/' || pathname.startsWith('/v2/ghcr.io/')) {
    const upstreamPath = REGISTRIES.ghcr.rewritePath(pathname);
    return { registry: REGISTRIES.ghcr, upstreamPath, proxyPath: pathname };
  }
  if (pathname === '/v2/quay.io') {
    return { registry: REGISTRIES.quay, upstreamPath: '/v2/', proxyPath: pathname };
  }
  if (pathname === '/v2/quay.io/' || pathname.startsWith('/v2/quay.io/')) {
    const upstreamPath = REGISTRIES.quay.rewritePath(pathname);
    return { registry: REGISTRIES.quay, upstreamPath, proxyPath: pathname };
  }
...
function registryFromAuthRequest(url) {
  ...
  const scope = url.searchParams.get('scope') || '';
  if (service === REGISTRIES.k8s.authService || scope.startsWith('repository:registry.k8s.io/')) {
    return REGISTRIES.k8s;
  }
  if (service === REGISTRIES.gcr.authService || scope.startsWith('repository:gcr.io/')) {
    return REGISTRIES.gcr;
  }
  if (service === REGISTRIES.ghcr.authService || scope.startsWith('repository:ghcr.io/')) {
    return REGISTRIES.ghcr;
  }
  if (service === REGISTRIES.quay.authService || scope.startsWith('repository:quay.io/')) {
    return REGISTRIES.quay;
  }
...
function isGoogleRegistryUpstream(host) {
  return (
    host === 'gcr.io' ||
...
function isK8sUpstream(host) {
  return host === 'registry.k8s.io';
}

function isGhcrUpstream(host) {
  return host === 'ghcr.io' || host === 'pkg-containers.githubusercontent.com';
}

function isQuayUpstream(host) {
  return host === 'quay.io' || host.endsWith('.quay.io');
}
...
function rewriteK8sImage(pathname) {
  if (pathname === '/v2/registry.k8s.io' || pathname === '/v2/registry.k8s.io/') return '/v2/';
  return pathname.replace(/^\/v2\/registry\.k8s\.io(?=\/)/, '/v2');
}

function rewriteGhcrImage(pathname) {
  if (pathname === '/v2/ghcr.io' || pathname === '/v2/ghcr.io/') return '/v2/';
  return pathname.replace(/^\/v2\/ghcr\.io(?=\/)/, '/v2');
}

function rewriteQuayImage(pathname) {
  if (pathname === '/v2/quay.io' || pathname === '/v2/quay.io/') return '/v2/';
  return pathname.replace(/^\/v2\/quay\.io(?=\/)/, '/v2');
}
...
function infoPage(host) {
  const examples = [
    ...
    `docker pull ${host}/registry.k8s.io/pause:3.9`,
    `docker pull ${host}/ghcr.io/actions/actions-runner:latest`,
    `docker pull ${host}/quay.io/prometheus/prometheus:latest`,
  ].join('\n');
  return `<!doctype html>
<html lang="zh-CN">
<head>
...
<title>Docker Hub / GCR / Kubernetes / GHCR / Quay Mirror</title>
<style>
...
<body>
  <div class="card">
    ...
    <h1>Docker Hub / GCR / Kubernetes / GHCR / Quay Mirror</h1>
    <p class="sub">基于 Cloudflare Worker 的 Docker Hub / GCR / registry.k8s.io / GHCR / Quay 拉取反向代理。仅支持 <code>docker pull</code>。</p>
  ...
  "registry-mirrors": ["https://${host}"]
    ...
    <p><small>健康检查：<code>GET /v2/</code> &nbsp;|&nbsp; token：<code>/token</code> &nbsp;|&nbsp; 上游：registry-1.docker.io / gcr.io / registry.k8s.io / ghcr.io / quay.io</small></p>
